defmodule SSCMEx.Examples.CameraInference do
  @moduledoc """
  Full example: start camera, load model, grab a frame, run inference, get outputs.
  ...
  `preset_idx: 0` for a smaller resolution, or a longer warm-up. "ion allocated failed" means
  out of ION memory — ensure NIF built with vb_blk_num = 1 and start camera before loading the model.
  """
  ...
  @doc """
  One-shot example: start camera, load model, capture one frame, run inference, return detections.

  ## Options

  - `:model_path` (required) - Path to the .cvimodel file
  - `:preset_idx` - Camera preset index (default 0). Use 1 for 1920x1080 @ 30fps.
  - `:threshold_score` - Detection score threshold 0.0–1.0 (default 0.5)
  - `:threshold_nms` - NMS threshold 0.0–1.0 (default 0.45)
  - `:warm_up_ms` - Delay after start_stream before first retrieve (default 3000). Increase if dmesg shows "vi err".

  ## Returns

  - `{:ok, %{frame: frame, detections: detections, perf: perf}}` on success
  ...
  Or use `run_once!/2` to raise on failure (so you get a clear error instead of MatchError):
      ...
      result = SSCMEx.Examples.CameraInference.run_once!(path, preset_idx: 0)
  """
  def run_once(model_path, opts \\ []) do
    preset_idx = Keyword.get(opts, :preset_idx, 0)
    threshold_score = Keyword.get(opts, :threshold_score, 0.5)
    threshold_nms = Keyword.get(opts, :threshold_nms, 0.45)
    warm_up_ms = Keyword.get(opts, :warm_up_ms, 3000)

    with {:ok, device} <- SSCMEx.Device.get_instance(),
         {:ok, 1} <- SSCMEx.Camera.count(device),
         {:ok, camera} <- SSCMEx.Camera.get(device, 0),
         # Start camera first (recommended when sharing ION with TPU)
         {:ok, :initialized} <- SSCMEx.Camera.init(camera, preset_idx),
         ...
         :ok <- SSCMEx.Model.set_config(model, :threshold_nms, threshold_nms),
         # Grab frame (retry a few times; first frame can be delayed)
  ...
  Order matters when using camera + TPU (shared ION memory):
  start camera first, then load engine/model.
  """
  def steps do
    """
    ...
    # 3. Start camera (init + stream) — do this before loading the model when using both
    {:ok, :initialized} = SSCMEx.Camera.init(camera, 0)   # or preset 1 for 1920x1080
    {:ok, :streaming} = SSCMEx.Camera.start_stream(camera, :refresh_on_return)

    # 4. Start model (engine + load + create)
    {:ok, engine} = SSCMEx.Engine.new()
    ...
    # frame is %{width: w, height: h, format: :rgb888, data: binary, ...}

    # 6. Run frame through model
    image = SSCMEx.Image.new(frame.width, frame.height, frame.format, frame.data)
    {:ok, detections} = SSCMEx.Model.run(model, image)

    # 7. Get outputs (for a detector: list of %{x, y, w, h, score, target})
  ...
  defp run_frame(model, %{width: w, height: h, format: f, data: data}) do
    image = SSCMEx.Image.new(w, h, f, data)
    SSCMEx.Model.run(model, image)
  end
  ...
  defp retrieve_frame_with_retry(camera, attempts, delay_ms) when attempts > 0 do
    case SSCMEx.Camera.retrieve_frame(camera, :rgb888) do
      {:ok, frame} -> {:ok, frame}
      _ -> Process.sleep(delay_ms); retrieve_frame_with_retry(camera, attempts - 1, delay_ms)
    end
  end
