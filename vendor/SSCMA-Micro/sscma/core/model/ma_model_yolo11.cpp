...
Yolo11::Yolo11(Engine* p_engine_) : Detector(p_engine_, "Yolo11", MA_MODEL_TYPE_YOLO11) {
    ...
    num_record_ = (h >> 3) * (w >> 3) + (h >> 4) * (w >> 4) + (h >> 5) * (w >> 5);
...
bool Yolo11::isValid(Engine* engine) {
    ...
    for (size_t i = 0; i < 6; i += 2) {
        ...
        int expected_h = h >> ((i / 2) + 3);
        int expected_w = w >> ((i / 2) + 3);
        if (box_shape.size != 4 || box_shape.dims[0] != 1 || box_shape.dims[1] != 64 || box_shape.dims[2] != expected_h || box_shape.dims[3] != expected_w) {
            return false;
        }
        if (cls_shape.size != 4 || cls_shape.dims[0] != 1 || cls_shape.dims[2] != expected_h || cls_shape.dims[3] != expected_w) {
            return false;
        }
    }
